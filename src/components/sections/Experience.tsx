    ...
    <section id="experience" className="relative scroll-mt-24 py-28">
      <div className="container-px">
        ...
        <div ref={ref} className="relative mt-16 pl-8 sm:pl-12">
          {/* Timeline rail */}
          <div className="absolute left-[7px] top-2 h-full w-px bg-line/10 sm:left-[15px]" />
          <motion.div
            ...
            {experiences.map((exp, ei) => (
              <div key={exp.company} className="relative">
                ...
                <motion.span
                  ...
                  className="absolute -left-8 top-1.5 grid h-4 w-4 place-items-center rounded-full bg-accent shadow-glow ring-4 ring-surface sm:-left-12"
                >
                ...
                <Reveal>
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                    <h3 className="font-display text-2xl font-bold text-strong">
                      {exp.company}
                    </h3>
                  ...
                  <p className="mt-3 max-w-2xl text-base">{exp.summary}</p>
                </Reveal>
                  ...
                  {exp.projects.map((project, pi) => (
                    ...
                    >
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        <h4 className="font-display text-lg font-semibold text-strong">
                          {project.name}{' '}
                          <span className="text-sm font-normal text-accent-soft">
                            — {project.subtitle}
                          </span>
                        </h4>
                        <span className="text-xs text-faint">{project.role}</span>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {project.technologies.slice(0, 6).map((tech) => (
                          <span
                            key={tech}
                            className="rounded-md bg-card/[0.05] px-2 py-0.5 text-xs text-base"
                          >
                            {tech}
                          </span>
                        ))}
